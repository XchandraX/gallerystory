...
@section('title', 'Private Gallery')

@section('content')
    <div class="flex justify-between items-center mb-8">
        <h1 class="text-3xl font-bold text-gray-900">Private Gallery</h1>
        <div class="flex items-center space-x-4">
            <x-sort-dropdown currentSort="{{ $sort ?? 'terbaru' }}" />
            
            {{-- Tombol Logout Private --}}
            <form action="{{ route('private.logout') }}" method="POST" class="inline">
                @csrf
                <button type="submit" class="bg-red-500 text-white px-4 py-2 rounded-md hover:bg-red-600 transition shadow-md hover:shadow-lg flex items-center space-x-2">
    ...
    @if($photos->hasMorePages())
        <div class="text-center mt-12">
            <button id="load-more" 
                    data-url="{{ route('private.gallery') }}" 
